...
type StakeTxHandler struct {
}

//_______________________________________________________________________

// InitState - set genesis parameters for staking
func (h *StakeTxHandler) InitState(key, value string, store state.SimpleDB) error {
...
func (h *StakeTxHandler) setValidator(value string, store state.SimpleDB) error {
...
func (h *StakeTxHandler) CheckTx(ctx types.Context, store state.SimpleDB, tx sdk.Tx) (res sdk.CheckResult, err error) {
...
func (h *StakeTxHandler) DeliverTx(ctx types.Context, store state.SimpleDB, tx sdk.Tx) (res sdk.DeliverResult, err error) {
	sender, err := h.getTxSender(ctx)
	if err != nil {
		return
	}
...
func (h *StakeTxHandler) getTxSender(ctx types.Context) (sender common.Address, err error) {
...
func (c check) declareCandidacy(tx TxDeclareCandidacy) error {
	...
	// check to see if the associated account has 10%(RRR, short for Reserve Requirement Ratio, configurable) of the max staked amount
...
func (c check) updateCandidacy(tx TxUpdateCandidacy) error {
	...
	// If the max amount is updated, the 10% of self-staking will be re-computed,
...
func (c check) delegate(tx TxDelegate) error {
	...
	// check to see if the validator has reached its declared max amount to be staked.
	x := new(big.Int)
...
func (d deliver) declareCandidacy(tx TxDeclareCandidacy) error {
	...
	// delegate a part of the max staked amount
	...
	if tx.MaxAmount != "" {
		...
		if candidate.ParseMaxShares().Cmp(maxAmount) != 0 {
			...
			if rechargeAmount.Cmp(big.NewInt(0)) > 0 {
				// charge
				commons.Transfer(d.sender, constant.StakeAccount, rechargeAmountAbs)
				...
				delegation.UpdatedAt = utils.Now()
...
func (d deliver) withdrawCandidacy(tx TxWithdrawCandidacy) error {
	...
	// Self-staked tokens will be refunded back to the validator address.
	delegations := GetDelegationsByPubKey(candidate.PubKey)
	for _, delegation := range delegations {
		err := commons.Transfer(d.params.StakeAccount, delegation.DelegatorAddress, delegation.Shares())
		if err != nil {
			return err
		}
...
func (d deliver) delegate(tx TxDelegate) error {
	...
	err := commons.Transfer(d.sender, d.params.StakeAccount, delegateAmount)
	if err != nil {
		return err
	}
...
func (d deliver) withdraw(tx TxWithdraw) error {
	...
	return commons.Transfer(d.params.StakeAccount, d.sender, amount)
}
